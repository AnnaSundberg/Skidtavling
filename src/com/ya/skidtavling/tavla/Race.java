			...
			for (Participants p : compBoard.participantsList) {// ställer upp listan
				...
				if (!p.isNotFinished()) {
					...
					if (!p.isPassedMiddle() && p.getDistance() >= trackDistance / 2) {
						if (!p.isPassedMiddle() && p.getDistance() <= (trackDistance / 2) + 5) {// vet inte om detta gör
																								// så mkt längre
					...
					if (p.getDistance() >= trackDistance) {// gissar på att denna är den som styr loppets längd. testa
															// om det går att göra det längre med 500 istället
